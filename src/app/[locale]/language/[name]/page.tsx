...
import { languajeToDefault } from "@/utils/traslate-force";
export default async function NameLanguage({
    params,
    searchParams,
}: {
    params: { name: string }
    ...
    const t = await getTranslations('LanguageName');

    await connectToMongoDB();
    const repositoryObra = new RepositoryObra();
    let paramName = deserializeSlug(params.name);
    paramName = languajeToDefault(paramName)
    ...
    const { count, obras } = await repositoryObra.GetAllPaginate({
        query: { 'lenguaje.nombre': paramName },
        sort: { 'fecha': 'desc' },
        page: page,
        take: numberSaltPage
    })
    ...
    return (
        <>
            <div className="grid grid-cols-1 gap-4">
                <TitleGeneral
                    name={`${t('title')}${paramName.toUpperCase()}`}
                />
            </div>
            <div className="grid">
                <Pagination
